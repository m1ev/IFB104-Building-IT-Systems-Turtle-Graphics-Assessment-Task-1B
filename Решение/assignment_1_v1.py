...
# Create 2 mirrored images of our shark to correctly display its
# movements on canvas
def create_shark_turtle():
    ...
    # Start drawing...
    # Hexagon
    test_shark.goto(hex_start_x, hex_start_y)
    test_shark.fillcolor('#00FFFF')
    test_shark.begin_fill()
    test_shark.begin_poly()
    test_shark.pensize(2)
    test_shark.pendown()
    for border in range(6):
        test_shark.forward(60)
        test_shark.left(300)
    test_shark.end_poly()
    test_shark.end_fill()
    hexagon = test_shark.get_poly()
    ...
    # Create a compound shape of our shark and of its mirrored image
    shark = Shape("compound")
    shark.addcomponent(hexagon, "#00FFFF", "black")
    ...
    shark2.addcomponent(hexagon_2, "#00FFFF", "black")
...
def make_move(shark_type, direction, x, y, is_moving):
    speed('slowest')
    tracer(True)
    goto(x, y)
    shape(shark_type)
...
def visualise_data(data_set):
    ...
    # Create 2 mirrored shapes of a shark
    ...
    for move in data_set:
        ...
        if move[0] == 0:
            ...
            x_new, y_new = make_move(moves[ind][1], 
                                     moves[ind][0], 0, 0, False)
        ...
        x_new, y_new = make_move(moves[ind][1], moves[ind][0], 
                                 x_new, y_new, True)
